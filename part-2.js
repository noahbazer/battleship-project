...
const handleHit = function (row, col, userGuess) {
  ...
  let hitShip = boardShips.find((ship) =>
    ship.locations.some(
      (location) => location[0] === col && location[1] === row
    )
  );

  //Removes hit location from ship's array, and checks to see if ship is sunk
  hitShip.locations = hitShip.locations.filter(
    (location) => location[0] !== col || location[1] !== row
  );
  if (hitShip.locations.length === 0) {
    ...
    if (remainingShips === 1) {
      console.log(
        chalk.yellowBright(`Ship sunk! ${remainingShips} ship remaining!`)
      );
    } else {
      console.log(
        chalk.yellowBright(`Ship sunk! ${remainingShips} ships remaining!`)
      );
    }
  }
};
...
const inputHandle = function (userGuess, isValid) {
  if (userGuess.toUpperCase() === "SHIPS") {
    console.log(
      chalk.cyanBright(`There are ${remainingShips} ships remaining.`)
    );
  } else if (!isValid) {
...
const logAttempts = function () {
  ...
  if (placeAttempts < 40) {
    ...
    consoleAttempts = chalk.yellowBright(
      `${placeAttempts} Placement attempts,`
    );
  } else {
...
while (remainingShips > 0) {
  let userGuess = readlineSync
    .question(
      "Enter a location to strike (or 'SHIPS' to see remaining ships): "
    )
    .toUpperCase();
  ...
  if (remainingShips === 0) {
    let playAgain = readlineSync.keyInYNStrict(
